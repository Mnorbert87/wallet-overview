...
const DICT: Record<string, [string, string]> = {
  ...
  // B3: az in-app lista már ÉL (Blockscout Approval-logok, 6 lánc, kulcs nélkül)
  "approvals.body": ["Az aktív ERC-20 engedélyek (mit engedélyeztél és kinek) a pénzed legnagyobb rejtett kockázata. Az alábbi lista a valós Approval-eventekből épül (6 lánc, kulcs nélkül); visszavonni a revoke.cash-en tudod — a címed előre kitöltve.",
                     "Active ERC-20 approvals (what you allowed and to whom) are your biggest hidden risk. The list below is built from real Approval events (6 chains, keyless); revoke on revoke.cash — your address is pre-filled."],
  "approvals.cta": ["Visszavonás a revoke.cash-en →", "Revoke on revoke.cash →"],
  "approvals.loading": ["Approval-eventek lekérése (6 lánc, Blockscout)…", "Fetching approval events (6 chains, Blockscout)…"],
  "approvals.none": ["Nincs aktív ERC-20 engedély ezen a címen a vizsgált láncokon. ✓", "No active ERC-20 approvals for this address on the scanned chains. ✓"],
  "approvals.unavailable": ["Az approval-adatforrás most nem elérhető — próbáld újra később; kitalált adatot nem mutatunk.",
                            "Approval data source is currently unavailable — try again later; we never show fabricated data."],
  "approvals.count": ["{n} aktív engedély", "{n} active approvals"],
  "approvals.colToken": ["Token", "Token"],
  "approvals.colSpender": ["Spender", "Spender"],
  "approvals.colAmount": ["Keret", "Allowance"],
  "approvals.colDate": ["Engedélyezve", "Approved"],
  "approvals.unlimited": ["korlátlan ⚠", "unlimited ⚠"],
  "approvals.unlimitedTip": ["Korlátlan keret — a spender a teljes token-egyenleged elmozgathatja. Ha nem használod aktívan, vond vissza.",
                             "Unlimited allowance — the spender can move your entire token balance. Revoke if not actively used."],
  "approvals.eventNote": ["A keret a legutóbbi Approval eventből származik — a spender által már felhasznált rész nincs levonva (a pontos maradék allowance-hoz per-pár lekérdezés kellene).",
                          "Allowance comes from the latest Approval event — the portion already spent by the spender is not deducted (exact remaining allowance would need per-pair queries)."],
  "approvals.truncated": ["⚠ {chains}: a log-limit miatt a lista csonka lehet ezen a láncon.",
                          "⚠ {chains}: the list may be truncated on this chain due to the log limit."],
  "approvals.chainsErrored": ["⚠ {chains}: most nem elérhető — ezeken a láncokon az engedélyek ISMERETLENEK (nem \"nincs\").",
                              "⚠ {chains}: currently unavailable — approvals on these chains are UNKNOWN (not \"none\")."],
  "approvals.moreRows": ["Az első 40 sor látszik ({n} összesen).", "Showing first 40 rows ({n} total)."],
